...
const mockHandloomProducts: HandloomItem[] = [
  {
...
export class HandloomService {
  /**
   * List products with tribe, category, and search query filters
   */
  static async getProducts(filters: { tribe?: string; category?: string; query?: string }) {
    try {
   ...
   */
  static async logInquiry(data: { productId?: string; buyerName?: string; buyerPhone?: string; channel?: string }) {
    try {
      await supabaseAdmin.from('orders_inquiries').insert({
